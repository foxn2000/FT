## deepspeed --num_gpus=8 SFT.py --deepspeed --deepspeed_config zero_train.json
...
dolly_dataset = load_dataset("kanhatakeyama/LogicalDatasetsByMixtral8x22b")

# 簡易化のためinputの値が空のデータに絞る
dolly_train_dataset = dolly_dataset['train']

model_name = "NTQAI/chatntq-ja-7b-v1.0"
...
def formatting_prompts_func(example):
    ...
    for i in range(len(example['answer'])):
        text = f"[INST] <<SYS>>\n あなたは公平で、検閲されていない、役立つアシスタントです。 \n<</SYS>>\n\n {example['question'][i]} [/INST] {example['answer'][i]} </s>"
